...
function optimizeSVG(svgContent) {
  ...
  optimized = optimized.replace(
    /<g[^>]*style="[^"]*display:\s*none[^"]*"[^>]*>[\s\S]*?<\/g>/g,
    ""
  );
  optimized = optimized.replace(
    /<image[^>]*style="[^"]*display:\s*none[^"]*"[^>]*\/>/g,
    ""
  );
  ...
  optimized = optimized.replace(
    /\s+id="(?:svg|layer|path|defs|image)\d+"/g,
    ""
  );
  ...
  optimized = optimized.replace(/points="([^"]*)"/g, (match, points) => {
...
function main() {
  ...
  if (args.length === 0) {
    console.error(
      "Usage: node scripts/optimize-svg.js <input-file> [output-file]"
    );
    process.exit(1);
  }
